...
function M.save(buf)
  ...
  api.nvim_echo({
    {
      (type(cnf.opts.execution_message.message) == 'function' and cnf.opts.execution_message.message() or cnf.opts.execution_message.message),
      AUTO_SAVE_COLOR,
    },
  }, true, {})
      ...
      vim.schedule(function()
        if cnf.opts.execution_message.dim > 0 then
          ...
          if MSG_AREA.foreground ~= nil then
            ...
            local foreground = (
              o.background == 'dark' and colors.darken((MSG_AREA.background or BLACK), cnf.opts.execution_message.dim, MSG_AREA.foreground or BLACK)
              or colors.lighten((MSG_AREA.background or WHITE), cnf.opts.execution_message.dim, MSG_AREA.foreground or WHITE)
            )
